/* Audio transport: getting a block of PCM from C into the browser's speakers.
 *
 * This file knows nothing about m4a.  Its whole job is "here are N stereo
 * frames, play them", and the reason it is separate from audio.c is that the
 * two failed independently while this was being written: a mixer that produces
 * silence and a transport that drops everything look identical from the couch.
 *
 ...
 * PortAudioFrameSamples() in audio.c.
 */
...
/* The ring in the worklet, in frames.  Four game frames is ~67 ms: long enough
 * that a single slow main-thread frame (a room transition decompressing
 * tilesets, say) does not become an audible hole, short enough that input
 * still feels attached to its sound effect. */
...
/* Frames the device still has to play.  Used only for clock feedback, so a
 * slightly stale answer is harmless -- but it must never read as zero while
 * audio is queued, or the producer widens every block and drifts long. */
 ...
 * ever changes. */
EM_JS(void, PortAudioPushJs, (const float *samples, int frames), {
    var pa = Module.portAudio;
    if (!pa) return;
    var n = frames * 2;
 ...
 * is up to ~1600 samples and the mixer already runs deep under Asyncify. */
...
void PortAudioPush(const s16 *interleaved, int frames)
{
    ...
    /* s16 -> float here rather than in JS: the conversion is one multiply per
     * sample and doing it on this side keeps the JS free of any assumption
     * about the mixer's output format. */
    for (i = 0; i < frames * 2; i++)
        sOut[i] = (float)interleaved[i] * (1.0f / 32768.0f);

    PortAudioPushJs(sOut, frames);
}
